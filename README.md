# Role of nonstabilizerness in quantum Born machines
...
python_codes/
  ...
  get_table_1.py, get_data_table_1.py   same for the appendix table (independent 2D chains)
...
Figures 1–5 are in the main text; Figures 6–7 and Table I are in Appendix B.
...
| 3 | SK-Gibbs entangler comparison vs temperature | `get_figure_3.py` | `get_data_figure_3.py` | `run_gibbs.py`, `compute_gibbs_randphase.py` |
...
| 6 | KLD vs depth, four entanglers on the six 2D targets, independent chains (Appendix B) | `get_figure_6.py` | `get_data_figure_6.py` | `run_twod_chains.py` |
| Table I | Converged KLD at L=12 for the same runs (Appendix B) | `get_table_1.py` | `get_data_table_1.py` | `run_twod_chains.py` |
| 7 | Operator entanglement per cut; register-cut entropy per layer (Appendix B) | `get_figure_7.py` | `get_data_figure_7.py` | `run_twod_chains.py` |
...
@article{plodzien_qsbm_nonstabilizerness,
  title  = {Role of nonstabilizerness in quantum Born machines},
